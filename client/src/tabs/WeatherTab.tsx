    ...
    <div className="space-y-6">
      ...
      <Card className="dark-card">
        <CardHeader className="dark-card-header">
          <CardTitle className="text-blue-400">{translations.currentWeather}</CardTitle>
        </CardHeader>
        <CardContent className="p-6">
          {isLoadingCurrent ? (
            <div className="flex flex-col space-y-4">
              <Skeleton className="h-12 w-1/3 bg-[#333]" />
              <Skeleton className="h-8 w-1/4 bg-[#333]" />
              <Skeleton className="h-6 w-2/4 bg-[#333]" />
            ...
            <>
              <div className="flex flex-col md:flex-row items-center">
                <div className="text-center md:text-left mb-4 md:mb-0">
                  ...
                  <div className="text-lg text-gray-200">{currentWeather.description}</div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                <div className="text-center p-3 bg-[#252525] border border-[#333] rounded-lg">
                  <Thermometer className="mx-auto text-blue-400 mb-2" />
                  <div className="text-sm text-gray-400">{translations.feelsLike}</div>
                  <div className="font-medium text-gray-200">
                    {currentWeather.feelsLike}°{language === 'es' ? 'C' : 'F'}
                  </div>
                </div>
                <div className="text-center p-3 bg-[#252525] border border-[#333] rounded-lg">
                  <Wind className="mx-auto text-blue-400 mb-2" />
                  <div className="text-sm text-gray-400">{translations.wind}</div>
                  <div className="font-medium text-gray-200">
                    {currentWeather.windSpeed} {language === 'es' ? 'km/h' : 'mph'}
                  </div>
                </div>
                <div className="text-center p-3 bg-[#252525] border border-[#333] rounded-lg">
                  <Droplets className="mx-auto text-blue-400 mb-2" />
                  <div className="text-sm text-gray-400">{translations.humidity}</div>
                  <div className="font-medium text-gray-200">{currentWeather.humidity}%</div>
                </div>
                <div className="text-center p-3 bg-[#252525] border border-[#333] rounded-lg">
                  <Eye className="mx-auto text-blue-400 mb-2" />
                  <div className="text-sm text-gray-400">{translations.visibility}</div>
                  <div className="font-medium text-gray-200">
      ...
      <Card className="dark-card">
        <CardHeader className="dark-card-header">
          <CardTitle className="text-blue-400">{translations.forecast}</CardTitle>
        </CardHeader>
        <CardContent className="p-6">
          {isLoadingForecast ? (
            <div className="flex space-x-4 overflow-x-auto">
              {[...Array(5)].map((_, i) => (
                <Skeleton key={i} className="flex-shrink-0 w-32 h-32 bg-[#333]" />
              ))}
            </div>
          ) : forecast ? (
            <div className="overflow-x-auto">
              <div className="flex space-x-4">
                {forecast.days.map((day, index) => (
                  <div key={index} className="flex-shrink-0 w-32 text-center p-3 bg-[#252525] border border-[#333] rounded-lg">
                    <div className="font-medium text-blue-300 mb-2">{day.name}</div>
                    <WeatherIcon condition={day.condition} />
                    <div className="text-sm">
                      <span className="font-medium text-gray-200">{day.highTemp}°</span> / <span className="text-gray-400">{day.lowTemp}°</span>
                    </div>
                  </div>
                ))}
              </div>
